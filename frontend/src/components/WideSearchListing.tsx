            ...
            !results ?
                [...Array(10)].map(i => {
                    return <ListItem key={i}>
                        <SearchItemSkeleton animation="pulse" cardProps={{ style: { width: "100%" } }} />
                    </ListItem>
                })
                :
                results?.map((value, idx) => {
                    return <ListItem key={idx.toString()}>
                        ...
                        <Link to={`../holdings/${value.id?.slice(value.id.length - 32)}`} state={value} style={{ textDecoration: 'none', width: "100%" }}>
